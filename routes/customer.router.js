...
router.post('/', checkScopes('admin'), validatorHandler(createCustomerSchema, 'body'), async (req, res, next) => {
	try {
		const body = req.body;
...
router.delete('/:id', checkScopes('admin'), validatorHandler(getCustomerSchema, 'params'), async (req, res, next) => {
	try {
		const { id } = req.params;
